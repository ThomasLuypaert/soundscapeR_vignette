...
-   **Phase 3:** Compare the diversity of multiple soundscapes using the `ss_compare`, `ss_pcoa`, `ss_divpart`, and `ss_pairdis` functions\
    \
...
print(paste0("First day of data collection: ", soundscape_list[["Andre"]]@first_day))
print(paste0("Latitude at data collection site: ", soundscape_list[["Andre"]]@lat))
print(paste0("Longitude at data collection site: ", soundscape_list[["Andre"]]@lon))
print(paste0("Time zone at data collection site: ", soundscape_list[["Andre"]]@tz))
print(paste0("Sunrise at time of data collection: ", soundscape_list[["Andre"]]@sunrise))
print(paste0("Sunset at time of data collection: ", soundscape_list[["Andre"]]@sunset))
...
print(paste0("Where are the raw sound files located: ", soundscape_list[["Andre"]]@fileloc))
print(paste0("What acoustic index are we using: ", soundscape_list[["Andre"]]@index, " index"))
print(paste0("What was the samplerate used to collect the data: ", soundscape_list[["Andre"]]@samplerate, " Hz"))
print(paste0("What was the window length used during the FFT: ", soundscape_list[["Andre"]]@window, " samples"))
...
head(soundscape_list[["Andre"]]@merged_df)[,1:5]
...
paste0("The data frame contains: ", ncol(soundscape_list[["Andre"]]@merged_df), " columns")

# What are the column names?

head(colnames(soundscape_list[["Andre"]]@merged_df))
tail(colnames(soundscape_list[["Andre"]]@merged_df))
...
paste0("The data frame contains: ", nrow(soundscape_list[["Andre"]]@merged_df), " rows")

# What do these row names look like?

  # The first five names

paste0("The first five rownames: ", paste0(rownames(soundscape_list[["Andre"]]@merged_df)[1:5], collapse = ", "))

  # The last five names

paste0("The last five rownames: ", paste0(rownames(soundscape_list[["Andre"]]@merged_df)[123:128], collapse = ", "))
...
paste0("The minimum CVR-value in our data frame is: ", min(soundscape_list[["Andre"]]@merged_df))

# What is the maximum CVR-index value in our data frame?

paste0("The max CVR-value in our data frame is: ", max(soundscape_list[["Andre"]]@merged_df))
...
paste0("The ", soundscape_list[["Andre"]]@binarization_method, " method was used for binarization")

# Which threshold was used for binarization?

paste0("The threshold used for binarization was: ", soundscape_list[["Andre"]]@threshold)
...
dim(soundscape_list[["Andre"]]@binarized_df)

# What are the unique values contained in this data frame?

unique(unlist(soundscape_list[["Andre"]]@binarized_df))
...
paste0("The aggregated_df data frame has ", 
       nrow(soundscape_list[["Andre"]]@aggregated_df), 
       " rows")

paste0("The aggregated_df data frame has ", 
       ncol(soundscape_list[["Andre"]]@aggregated_df), 
       " columns")
...
paste0("The relative abundance of OSUs can take the following values: ", 
       paste(sort(unique(unlist(soundscape_list[["Andre"]]@aggregated_df))), collapse = ","))
...
ss_heatmap(soundscape_obj = soundscape_list[["Andre"]])
...
ss_diversity(soundscape_obj = soundscape_list[["Andre"]], 
         qvalue = 0, 
         output = "raw")
...
ss_evenness(soundscape_obj = soundscape_list[["Andre"]])
...
head(ss_diversity(soundscape_obj = soundscape_list[["Andre"]], 
             qvalue = 0, 
             output = "percentage", 
             subset = "tod"), n = 24)
...
ss_diversity_plot(soundscape_obj = soundscape_list[["Andre"]], 
                  qvalue = 0, 
                  graphtype = "total", 
                  output = "percentage", 
                  smooth = FALSE)
...
plot_1 <- ss_diversity_plot(soundscape_obj = soundscape_list[["Andre"]], 
                  qvalue = 0, 
                  graphtype = "total", 
                  output = "percentage", 
                  smooth = TRUE, 
...
plot_2 <- ss_diversity_plot(soundscape_obj = soundscape_list[["Andre"]], 
                  qvalue = 0, 
                  graphtype = "total", 
                  output = "percentage", 
                  smooth = TRUE, 
...
plot_3 <- ss_diversity_plot(soundscape_obj = soundscape_list[["Andre"]], 
                  qvalue = 0, 
                  graphtype = "total", 
                  output = "percentage", 
                  smooth = TRUE, 
...
soundscapeR::ss_compare(soundscape_obj_A = soundscape_list[["Andre"]],
                        soundscape_obj_B = soundscape_list[["Mascote_A1"]], 
                        maxfreq = 20000)
...
- <span style="font-weight: bold;">soundscape_list:</span><br> A list of soundscape objects. Each object in the list should be produced using the \code{\link{ss_create}} function (or ss_index_merge, ss_binarize, and ss_aggregate in sequence).<br><br>
...
ss_pcoa(soundscape_list = soundscape_list)
...
- <span style="font-weight: bold;">grouping:</span><br> A numeric or character vector indicating potential grouping of the elements in the 'soundscape_list' object. Make sure that the grouping vector has the same length as the 'soundscape_list' argument.<br><br>
...
The *grouping* argument is used to indicate which soundscapes in the *soundscape_list* argument belong together, and colors them accordingly. This is already a little clearer than the previous plot. 
...
- <span style="font-weight: bold;">soundscape_list:</span><br> A list of soundscape objects of equal dimensions, each soundscape object being produced by ss_create (or ss_index_merge, ss_binarize and ss_aggregate in sequence)<br><br>
...
ss_divpart(soundscape_list = soundscape_list,
           qvalue = 0)
...
To use the `ss_pairdis` function, we need to supply the same parameters as the `ss_divpart` function: *soundscape_list*, *qvalue* and *hier_table* (optional). 

**Let's have a look:**

```{r, cache=TRUE}

ss_pairdis(soundscape_list = soundscape_list, 
           qvalue = 0)
